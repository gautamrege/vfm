...
typedef struct __eth_hdr{
        ...
        /* VLAN Tag (IEEE 802.1q) It is optional*/
        uint32_t vlan_tag;
...
typedef struct __fc_hdr{
        ...
        /*
         * Uniquely identifies frames in a non-streamed sequence or when only
         * one sequence is open
         */
        uint8_t seq_id;
        /*Set to 00h to indicate that no optional FC headers are used*/
        uint8_t data_field_ctrl;
        /*
         * Indicates the sequential order of frames within a sequence.
         * Start with 0000h
         */
        uint16_t seq_count;
...
typedef struct __mlx_tunnel_hdr {
        ...
        uint8_t  group;
        /* Port number MSB:1 = external, 0=internal */
        uint8_t  port_num;
        uint16_t reserved2;
        /* Determine from gateway initial advertisement */
        uint32_t gw_id;
        /* VFM id - set during association with bridgex */
        uint32_t vfm_id;
}mlx_tunnel_hdr;
...
typedef struct __fcoe_bridge_vfm_adv {
        ...
        /*
         * Gautam - Today this plugin support Mellanox. Tomorrow, if we want to
         * a build specific Cisco plugin, we need to simply set the compile
         * flag.
...
typedef struct __fcoe_vfm_bridge_adv_res{
        ...
        uint8_t node_name[NAME_LEN];    /* VFM Node Name   */
        uint16_t max_recv;              /* Max Receive Size*/
#ifdef MELLANOX
        mlx_adv_res res;
#endif
...
typedef struct __vHBA
{
        ...
        uint32_t port_id;                 /* vHBA Port ID:Lower 23 bits */
...
typedef struct __fcoe_vHBA_alive{
        ...
        uint8_t node_name[NAME_LEN];    /* Node Name */
        uint16_t max_recv;              /* Max Receive Size */
        vHBA hba;                       /* VHBA info: ConnectX vHBA */
}fcoe_vHBA_alive;
