 ...
 */
#ifndef INCLUDED_bxm_gateway
#define INCLUDED_bxm_gateway
...
typedef struct
{
     ...
     */
    bxm_vfabric_id_t *_vfabric_list;
     ...
     */
    bxm_vadapter_id_t *_vadapter_list;
    ...
    /**
     * @brief The GUID of the BXM of which the Gateway is associated with.
     *
     * @note This is a read-only property i.e. a user cannot edit it using
     * the gateway API. This property can only be modified by the system when
     * a gateway is associated to a BXM.
     */
 ...
 * @return Returns 0 on success, or an error code on failure.
 */
bxm_error_t bxm_gateway_edit_port_attr(bxm_gw_id_t gw_id,
				bxm_port_type_t port_type,
 ...
 * means that BXM can activate the gateway and start the services of the
 ...
 * ports.
 *
 * @note After the call BXM will activate the gateway and change the status to
 ...
 * @return Returns 0 on success, or an error code on failure.
 */
bxm_error_t bxm_gateway_online(bxm_gw_id_t gw_id);
 ...
 * means that BXM can disable the gateway and stop the services of the
 * gateway. It will also turn the vfabrics and the corresponding vadapters as
 * DISABLED.
 *
 * @note After the call BXM will deactivate the gateway and change the status
 ...
 * @return Returns 0 on success, or an error code on failure.
 */
bxm_error_t bxm_gateway_offline(bxm_gw_id_t gw_id);
 ...
 * @return Returns 0 on success, or an error code on failure.
 */
bxm_error_t bxm_gateway_select_inventory(bxm_gateway_attr_t attr,
			    bxm_gateway_bitmask_t bitmask,
			    uint32_t *num_result,
			    bxm_gw_id_t *result[]);
 ...
 * @return Returns 0 on success, or an error code on failure.
 */
bxm_error_t bxm_gateway_query_general_attr(bxm_gw_id_t gw_id,
			    bxm_gateway_bitmask_t bitmask,
 ...
 * @return Returns 0 on success, or an error code on failure.
 */
bxm_error_t bxm_gateway_query_port_attr(bxm_gw_id_t gw_id,
				bxm_port_type_t port_type,
				bxm_port_attr_bitmask_t *bitmask,
				bxm_port_attr_t *result);
#endif /* INCLUDED_bxm_gateway */
