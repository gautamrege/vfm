		...
		res_packet *op_arg)
{
	...
	vpsdb_add_resource(VPS_DB_VFABRIC, &vp_res);
        ...
        op_arg->size  = sizeof(bxm_vfabric_id_t);
        op_arg->data  = malloc(op_arg->size);
        memcpy(op_arg->data, vp_res.data, sizeof(bxm_vfabric_id_t));

        vps_trace(VPS_INFO, "Vfabric id: %d",
			*((bxm_vfabric_id_t*)vp_res.data));
 ...
 */
bxm_error_t
process_bxm_edit_vfabric(uint8_t *buff, uint32_t *ret_pos, res_packet *op_arg)
{
        bxm_vfabric_id_t vfabric_id;
        bxm_vfabric_attr_bitmask_t bitmask;
        bxm_vfabric_attr_t attr, attr_db;
        char query[1024];
        void * void_ptr;
        uint32_t count = 0;
	...
	if(count > 0)	
...
#ifdef BXM_TEST
        void_ptr = &attr;
        display(void_ptr, sizeof(bxm_vfabric_attr_t));
        void_ptr = &attr_db;
        display(void_ptr, sizeof(bxm_vfabric_attr_t));
#endif
        ...
        vps_trace(VPS_ENTRYEXIT, "Leaving process_bxm_edit_vfabric");
}
		                ...
		                res_packet *op_arg)
{
	...
	bxm_gateway_attr_t *gateway_attr;
	req_action req;
	char query[1024];
	void *stmt;
	vpsdb_resource res;
	 ...
	 */
	sprintf(query, "select * from bxm_vadapter_attr where vfabric_id = %d",
			vfabric_id);
...
/*
        ...
        if (attr.protocol == BXM_PROTOCOL_EN) {
		strcpy(query, " update bxm_vfabric_en_attr set ");

		sprintf(query, "%s update bxm_vfabric_en_attr set
				where id = %d;" , query, vfabric_id);
		
                memcpy(&(attr_db.en_attr), &(attr.en_attr),
				
                                sizeof(bxm_vfabric_en_attr_t));
