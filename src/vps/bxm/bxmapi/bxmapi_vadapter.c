                ...
                bxm_vadapter_id_t *vadapter_id)
{
        ...
        mesg_len = (sizeof(bxmapi_ctrl_hdr) + 2 * NAME_SIZE +
                        sizeof(bxm_protocol_t) + 2 * no_of_args);
        ...
        *vadapter_id = *(bxm_vadapter_id_t*)op_pack.data;
        free(op_pack.data);
        free(pack.data);
        return err;
}
...
bxm_error_t
bxm_vadapter_edit_general_attr(bxm_vadapter_id_t vadapter_id,
                ...
                bxm_vadapter_attr_t * attr)
{
        ...
        /* TYPE = 1 , vadapter_id */
        create_api_tlv(TLV_INT, sizeof(bxm_vadapter_id_t), &vadapter_id,
                        &offset);
                ...
                void * attr)
{
        ...
        mesg_len = (sizeof(bxmapi_ctrl_hdr) +
                        sizeof(bxm_vadapter_en_attr_bitmask_t) +
                        sizeof(bxm_vadapter_en_attr_t) + 2 * no_of_args);

        memset(&ctrl_hdr , 0, sizeof(bxmapi_ctrl_hdr));

        err = create_ctrl_hdr(BXMAPI_VADAPTER, BXM_EDIT_PROTOCOL_ATTR,
			mesg_len, &ctrl_hdr);
        if (err)
                return err;
        ...
        /* TYPE =TLV_BIT_MASK , bxm_protocol_t */
        create_api_tlv(TLV_BIT_MASK, sizeof(bxm_vadapter_en_attr_bitmask_t),
			bitmask, &offset);

        /* TYPE = TLV_VADP_ATTR, bxm_vadapter_attr_t */
        create_api_tlv(TLV_VADP_ATTR, sizeof(bxm_vadapter_en_attr_t),
			attr, &offset);
