...
extern vfm_init_info_packet init_info_pack;
 ...
 */
vps_error
parse_ib_header(uint *buff, uint32_t *offset, vfm_ib_hdr *ib_hdr)
{
        vps_error err = VPS_SUCCESS;

        /* Copy IB header from the input buffer */
        memcpy(ib_hdr, buff, sizeof(vfm_ib_hdr));
        *offset += sizeof(vfm_ib_hdr);
 ...
 */
vps_error
create_ib_header(vfm_ib_hdr *ib_hdr)
{
        vps_error err = VPS_SUCCESS;
 ...
 */
vps_error
create_mad_header(vfm_mad_hdr *mad_hdr)
{
        vps_error err = VPS_SUCCESS;

        vps_trace(VPS_ENTRYEXIT, "Entering create_mad_header");

        mad_hdr->base_version  = VFM_MGMT_BASE_VERSION;
        mad_hdr->mgmt_class    = VFM_MGMT_SUBN_ADM_CLASS;
        mad_hdr->class_version = 0x02;
        mad_hdr->R_method      = INSERTF(mad_hdr->R_method, 0,
                                VFM_MGMT_SUBN_ADM_METHOD_SET, 0, 7);
...
fill_mcmember_record(uint16_t pkey, uint8_t *port_gid,
                        vfm_mcmemeber_record *mc_record)
{
        vps_error err = VPS_SUCCESS;
...
create_mcast_mad_packet(uint16_t pkey, uint8_t *port_guid,
                        vfm_sa_mad_packet *sa_mad_packet)
{
        vps_error err  = VPS_SUCCESS;
        uint64_t comp_mask;
        vfm_mcmemeber_record mc_rec;
        ...
        memcpy(sa_mad_packet->data, &mc_rec, sizeof(vfm_mcmemeber_record));
...
vfm_send_gw_adv()
{
        ...
        vfm_sa_mad_packet mc_join;
        vfm_init_info_packet info_pack;
        uint8_t *buff;

        vps_trace(VPS_ENTRYEXIT, "Entering vfm_send_packet");

        memset(&mc_join, 0, sizeof(vfm_sa_mad_packet));
...
//        buff[control_hdr.desc_list_length] = 0;
        ...
        /* Send ib packet  */
        vfm_send_ib_packet(&info_pack, &control_hdr, buff);
...
/* Send GW advertisements*/
vps_error
vfm_send_uincast_gw_adv(eoib_conx_vfm_adv *solicit_adv)
{
 ...
 */
vps_error
vfm_send_ib_packet(vfm_init_info_packet *info_pack, ctrl_hdr *control_hdr,
                   uint8_t *buff)
{
        vps_error err = VPS_SUCCESS;
        vfm_ib_hdr ib_hdr;
        ...
        memcpy(packet_buff, &ib_hdr, sizeof(vfm_ib_hdr));
        offset += sizeof(vfm_ib_hdr);
        ...
        vps_trace(VPS_ENTRYEXIT, "Leaving  vfm_send_packet");
        return err;
}
