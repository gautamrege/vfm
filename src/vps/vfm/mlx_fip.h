...
typedef struct __mlx_gateway {
    ...
    uint32_t ports_stats;           /* TODO:External port status: 
...
typedef struct __mlx_bridgeX{
    ...
    uint16_t reserved;             /* ref: Table5, mac is allocated 2 DWORDS
                                      Hence we need a reserved field too */
    uint8_t last_bc_mac[6];        /* Last associated BC mac address*/
    mlx_gateway *gw_arr;           /* Pointer to the gateway array. Number of
