...
vps_error vpsdb_update(const char* query)
{
    ...
    if( rc != SQLITE_OK ){
        ...
        /* Free zErrMsg ONLY during errors. It is set to NULL during success */
...
vps_error add_external_ports(uint16_t gw_id, uint16_t *ext_ports)
{
    ...
    for (i = 0; i < 15; i++)
    {
        strcpy(insert_external_ports, "insert into external_ports(gw_id, \
                                        id, \
                                        max_speed, \
                                        current_speed, \
                                        status) values (");
        sprintf(insert_external_ports, "%s%d", insert_external_ports, gw_id);
        sprintf(insert_external_ports, "%s,%d", insert_external_ports, i+1);
        /* TODO: max_speed, current_speed need to be set right */
        sprintf(insert_external_ports, "%s,%d,%d", insert_external_ports, 0,0);
        sprintf(insert_external_ports, "%s,%d);", insert_external_ports, 
                                                ext_ports[i]);
        ...
        {
            vps_trace(VPS_ERROR, 
                  "Ignoring error in adding external port(%d) for gateway", i+1);
            /* TODO: Ensure if this is correct. Are we supposed to ignore this
             * error?? - Gautam */
            /* Reset the error */
            err = VPS_SUCCESS;
        }
...
vps_error add_gateway(const char* bridge_id, vpsdb_gateway *gw)
{
    ...
    sprintf(insert_gateway_query, "%s,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d", 
    ...
    if (VPS_SUCCESS != (err = add_external_ports(gw->gw_id, gw->ext_ports)))
    {
        vps_trace(VPS_ERROR, "Error in adding gateway to the database");
        goto out;
    }
...
vps_error add_bridge(vpsdb_resource *info)
{
    ...
    for(i = 0; i < info->count; i++)
    {
        ...
        /* Database Id */
        sprintf(insert_bridge_query, "%s,%d", insert_bridge_query, ptr->db_id);
        ...
        {
            vps_trace(VPS_ERROR, "Error in adding bridge to the database");
            goto out;
        }
...
int process_bridge(void *data, int num_cols, char **values, char **cols)
{
    ...
    vps_trace(VPS_ENTRYEXIT, "Entering process_bridge. Count: %d", rsc->count);
...
vps_error populate_gw_external_ports(vpsdb_gateway *gw)
{
    ...
    sprintf(query, "select id,status from external_ports where gw_id = %d;", 
                    gw->gw_id);
    ...
    if (VPS_SUCCESS != (err = vpsdb_read(query, 
                        ...
                        &rsc))) 
    {
        vps_trace(VPS_ERROR, "Could not get external port information");
        err = VPS_DBERROR;
        goto out;
    }
out:
...
vps_error vpsdb_get_resource(uint32_t type, vpsdb_resource *info, const char *name) 
{
    ...
    else if(type == VPS_DB_GATEWAY)
    {
        ...
        /* Get all the host, CNA and vHBA, vNIC info */
        if (VPS_SUCCESS != (err = populate_gateway_information(info, name)))
            goto out;
...
vps_error vpsdb_add_resource(uint32_t type, vpsdb_resource *rsc)
{
    ...
    {
        vps_trace(VPS_ERROR, "Invalid type for database insertion: %x", type);
        err = VPS_DBERROR_INVALID;
        goto out;
    }
