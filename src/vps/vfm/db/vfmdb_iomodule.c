...
get_iomodule_by_mac(uint8_t *mac)
{
        ...
        sprintf(query, "select * from vfm_io_module_attr "
                       "where mac = ?1;");
...
/*
 * This function adds the io_module in the database(Table : vfm_io_modules)
 * 
...
add_io_module(vpsdb_resource *res_io_module)
{
        ...
        void *stmt;
        void **args;
        char get_max_query[128] = "select max(id) from vfm_io_module_attr;";
        ...
        sprintf(insert_query, "insert into vfm_io_module_attr(id, name, "
...
populate_iomodule(vpsdb_io_module_t *io_module)
{
        ...
        sprintf(query, "select * from vfm_io_modules_attr"
