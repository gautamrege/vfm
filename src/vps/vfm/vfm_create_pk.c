...
/*
 * fcoe_vHBA_deregister From VFM to IO_Module.
 * TODO : Write code for creating the FIP packet by reading the values
 * from the database and filling up the packet structure to send
 * it from the VFM to the IO_Module.
 *
 */
vps_error
                ...
                uint8_t *desc_buff)
{
        ...
        /* Fill the mapping structure from the IO_Module FLOGI Request */
...
create_reject_els(fc_hdr fc_header)
{
        ...
        vps_trace(VPS_WARNING, "ELS reject sent");
