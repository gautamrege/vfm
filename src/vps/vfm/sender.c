                ...
                int length)
{
        ...
        if (sendto(send_sd, buff, length, 0, (struct sockaddr*)&g_socket_addr,
                ...
                uint32_t *offset)
{
        ...
        /*Copy dest host mac address and increament offset by MAC_ADDR_LEN */
 ...
 * [IN] *conx_mac   : Mac addresss of IO_Module.
                ...
                uint16_t max_recv)
{
        ...
        if (tunnel_flag == TRUE) {
                ...
                if (control_hdr->opcode == 2 && control_hdr->subcode == 1) {
                        /* FLOGI FWED PACKET */
                        fill_ether_hdr(buff, conx_mac, bridge_mac,
                                        &vlan_tag, &offset);
                }
                else {
                        /*fill ethernet haeader*/
                        fill_ether_hdr(buff, conx_mac, g_bridge_enc_mac,
                                        &vlan_tag, &offset);
                }
        }
 ...
 */
vps_error
send_fc_packet(uint8_t *vfm_mac,
                uint8_t *bridge_mac,
                mlx_tunnel_hdr *tunnel_hdr,
                uint8_t *desc_buff,
                uint32_t desc_len)
{
        ...
        /*
         * Tunnel Header Length + SOF length + Descriptor Length +
         * FC footer length*2
         */
