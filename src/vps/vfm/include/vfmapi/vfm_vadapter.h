...
/**
 * @file
 * @brief Sample VFM virtual I/O adapter API.
 *
...
typedef enum VAdapterInitiationTypeEnum
{
     ...
     *
     * @note This flag indicates that VFM should initiate the instantiation
     * of the vadapter on the host.
     */
    HOST_INITIATED 	= 0x1,

    /**
     * @brief Creation of virtual adapters is initiated by VFM.
     *
     * @note This flag indicates that VFM should allow the instantiation
...
vfm_vadapter_en_attr_bitmask_t;
...
vfm_vadapter_fc_attr_bitmask_t;
...
typedef struct
{
     ...
     *
     * @note The name of the vdapter should be unique within a VFM context.
     ...
     */
    vfm_assignment_type_t assignment_type;
	...
	vfm_vadapter_fc_attr_t fc_attr;
    };
 ...
 * adds the local configuration of a virtual I/O adapter on the VFM. The actual
 ...
 */
vfm_error_t vfm_vadapter_create(char *name, char *desc,
				vfm_protocol_t protocol,
 ...
 * @return Returns 0 on success, or an error code on failure.
 */
vfm_error_t vfm_vadapter_clone(char *name, char *desc,
				vfm_vadapter_id_t src_vadapter_id,
				vfm_vadapter_id_t *new_vadapter_id);
 ...
 * these APIs may call vfm_vadapter_edit_general_attrs with appropriate
 * bitmask.
 *
 * @return Returns 0 on success, or an error code on failure.
 */
vfm_error_t vfm_vadapter_edit_general_attr(vfm_vadapter_id_t vadapter_id,
 ...
 * 	struct vfm_vadapter_en_attr_t or vfm_vadapter_fc_attr_t based on
 * 	the protocol type.
 * @param[in] attr Pointer to the structure containing the attrs of the
 * 	virtual adapter that should be updated.
 *
 * @return Returns 0 on success, or an error code on failure.
 */
vfm_error_t vfm_vadapter_edit_protocol_attr(vfm_vadapter_id_t vadapter_id,
 ...
 * operation is the trigger to VFM to start the services of the vadapter. After
 * the call the VFM system can instantiate the vadapter (based on whether it's
 ...
 * @return Returns 0 on success, or an error code on failure.
 */
vfm_error_t vfm_vadapter_online(vfm_vadapter_id_t vadapter_id);
 ...
 * operation is the trigger to VFM to diable all services of the vadapter.
 * After the call the VFM system will deactivate the virtual adapter. The
 * vadapter object and its properties remains in the VFM's configuration in
 * OFFLINE/DISABLED state.
 *
 * NOTE: For vNIC VFM will send appropriate commands e.g. Context table
 ...
 * @return Returns 0 on success, or an error code on failure.
 */
vfm_error_t vfm_vadapter_offline(vfm_vadapter_id_t vadapter_id);
 ...
 * @return Returns 0 on success, or an error code on failure.
 */
vfm_error_t vfm_vadapter_query_general_attrs(vfm_vadapter_id_t vadapter_id,
			    vfm_vadapter_attr_bitmask_t *bitmask,
 ...
 * @return Returns 0 on success, or an error code on failure.
 */
vfm_error_t vfm_vadapter_query_protocol_attr(vfm_vadapter_id_t vadapter_id,
			    void *bitmask /* typecast according to protocol*/,
			    void *attr /* typecase according to protocol */);
 ...
 *
 * This destroys the specified vadapter from the VFM. If the vadapter is ACTIVE
 ...
 *     vfm_vadapter_destroy()  returns, and the vadapter ID will be
 *     invalid for use with any other functions.
 */
vfm_error_t vfm_vadapter_destroy(vfm_vadapter_id_t vadapter_id);
