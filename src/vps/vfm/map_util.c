...
add_entry_to_map(req_entry_map *entry)
{
    ...
    if( vfm_oxid_index >= REQ_ENTRY_MAP_LEN || g_req_map[vfm_oxid_index] != NULL )
    {
        vfm_oxid_index = 1 ;
    ...
    vfm_oxid_index++;
    vps_trace(VPS_INFO, "Adding map key: %d  added index : %x ", temp_index,g_req_map[temp_index]->oxid);
...
get_entry_from_map(uint16_t vfm_gen_oxid)
{
    ...
    /*Index For given oxid not null fill up out put parameters oxid and mac */
    if(g_req_map[vfm_gen_oxid] !=  NULL)
    {
