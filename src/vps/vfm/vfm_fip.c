...
get_tlv(uint8_t *msg_desc, vp_tlv *tlv)
{
        ...
        /*
         * The data in the tlv starts after the type and len values.
         * Hence we need to allocate 2 bytes lesser.
         */
        length = ((tlv->length * DWORD) - sizeof(tlv->length)
                        - sizeof(tlv->type));
        ...
        vps_trace(VPS_INFO, "TLV: type: %d, length: %d",
                        tlv->type, tlv->length);
...
get_tlv_value(vp_tlv *tlv, void *data)
{
        ...
        switch (tlv->type)
        {
                ...
                case TLV_1 :
                         ...
                         * value Hence we need to remove the unwanted bits.
                         */
                        memcpy(data, tlv->value + sizeof(uint8_t),
                                        sizeof(uint8_t));
                        break;
                ...
                case TLV_2 :
                         ...
                         * address which we can directly copy into the output
                         * parameter.
                         */
                ...
                case TLV_3 :
                         ...
                         * Hence we first need to convert it from network byte
                ...
                case TLV_4 :
                         ...
                         * Hence we first move to the location 2 bytes ahead to
                         * point to the actual NODENAME and then copy it to the
                         * output parameter.
                         */
                ...
                case TLV_5 :
                         ...
                         * Hence we first move to the location 2 bytes ahead to
                         * point to the actual FABRIC NAME and then copy it to
                ...
                case TLV_6 :
                         ...
                         * Hence we first need to convert it from network byte
                         * order to Host byte order. And then assign it to the
                         * output parameter.
                         */
                ...
                case TLV_11:
                        /*
                         * TYPE : 11 vHBA structure: It contains FCF Gateway
                         * MAC (48 bits), vHBA port ID(24 bit), and vHBA Name.
                         * The vHBA port ID is in network byte order, which
                ...
                case TLV_12:
                         ...
                         * value of max receive size. Hence we first move to
                         * the tlv->value pointer 2 bytes ahead to point to the
                         * FKA_ADV_PERIOD. And also the value is in network
                         * byte order, hence we also need to convert it from
                ...
                case MLX_TLV_2 :
                        ...
                        bridgeX_ptr->gw_arr =
                                (mlx_gateway *)malloc(bridgeX_ptr->gw_num *
                                                sizeof(mlx_gateway));
                        gateway_ptr = bridgeX_ptr->gw_arr;

                        /* Set the gateway pointer from the tlv value */
                        tmp_ptr = tlv->value + sizeof(uint16_t) +
                                (3 * sizeof(DWORD));

                        /*
                         * Iterate over the tlv-data and populate the gateway
                         * information
                         */
                        for (i = 0; i < bridgeX_ptr->gw_num; i++) {
                                memcpy(gateway_ptr + i, tmp_ptr,
                                                sizeof(mlx_gateway));
                                gateway_ptr->gw_id = ntohs(gateway_ptr->gw_id);
                                gateway_ptr->ports_stats =
                                        ntohl(gateway_ptr->ports_stats);
        ...
        /*
         * TODO: For now we are freeing the value here. Ideally, we should
         * pass an OWNERSHIP flag to this method which determines whether the
         * (IN) parameter should be manipulated or not
         */
        free(tlv->value);
...
{
        vps_error err = VPS_SUCCESS;
        vps_trace(VPS_ENTRYEXIT, "Entering fcoe_conx_discovery");
        ...
        vps_trace(VPS_ENTRYEXIT, "Leaving fcoe_conx_discovery");
        return err;
}
 ...
 *
 * Sent from IO_Module(unicast) to VFM.
 *
 * mesg_desc (IN)    - It contains the copy of the packet received
 *                                         from the network.
